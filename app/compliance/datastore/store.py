...
class ComplianceDataStore:
    ...
    def load_coverage_articles(self, articles: list[dict]) -> None:
        ...
        covered_icd: [...], noncovered_icd: [...]}. Same shape as the LCD
        record, so the agent needs no change when the MCD bulk download is
        wired into the refresh layer.
        """
        self._ensure_coverage_policy_table()
        cpt_rows, icd_rows, noncov_rows, title_rows = [], [], [], []
        pids = set()
        for art in articles:
        ...
        self.conn.executemany(
            "INSERT INTO coverage_policy (policy_id, title, contractor) VALUES (?,?,?)",
            title_rows,
        )
        self.conn.executemany("INSERT INTO lcd_qualifying VALUES (?,?)", icd_rows)
        self.conn.commit()
    ...
    def coverage_policy_states(self, policy_id: str) -> set[str] | None:
        """States where the policy's issuing MAC adjudicates claims (resolved
        via mac_jurisdictions.json). None = can't be narrowed (unknown or
        missing contractor) — caller must treat the policy as potentially
        applicable everywhere."""
        from app.compliance import geo
        row = self.conn.execute(
            "SELECT contractor FROM coverage_policy WHERE policy_id=? LIMIT 1",
            (policy_id,),
        ).fetchone()
        if row is None:
            return None
