...
designations and CMS PFS attributes), not in the PTP edit tables — which is
...
All designations are read from each code's own descriptor / PFS attributes —
no curated code lists or numeric section proxies.
"""
...
class SurgicalPackageAgent(ComplianceAgent):
    ...
    def check(self, claim: Claim) -> list[Finding]:
        ...
        surgical = [
            ln for ln in cpt_lines
            if self.store.is_surgical_procedure(ln.code, claim.date_of_service)
        ]
        separation_modifiers = (
        ...
        anesthesia = [
            ln for ln in cpt_lines
            if self.store.is_anesthesia_service(ln.code, claim.date_of_service)
        ]
