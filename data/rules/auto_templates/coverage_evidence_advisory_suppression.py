...
suppresses_clause (string, REQUIRED for clause-emitting filters)
    The finding CLAUSE this suppression was verified against (the
    scrubber's findings carry a machine `clause` field, e.g. a
    medical-necessity filter emits distinct clauses for its
    class-findings-modifier advisory vs its claim-composition gate).
    Matching is exact: the rule retires ONLY the clause it names; a rule
    without this field matches only findings that carry no clause.
    Never name a clause the adjudicated dispute did not verify — a
    suppression must retire the specific assertion it has authority
    over, not every assertion the filter makes about the code.

also_suppresses_validator_categories (list of strings, optional)
    Validator-issue categories that assert the SAME adjudicated defect
    as the suppressed scrub advisory (two emissions of one claim about
    one code). Matching WARNING issues on the same code are removed at
    the validator's report build, with an auditable correction recorded
    — so the record never ships the advisory both suppressed and
    active. List a category ONLY when the adjudicated verdict covers
    its assertion too.
...
def execute(engine, rule, icd, cpt, hcpcs, coding_result,
    ...
    clause = rule.get("suppresses_clause")
    clause = clause.strip() if isinstance(clause, str) else ""
    vcats = rule.get("also_suppresses_validator_categories")
    if not isinstance(vcats, list):
        vcats = []
    vcats = [c for c in vcats if isinstance(c, str) and c.strip()]
