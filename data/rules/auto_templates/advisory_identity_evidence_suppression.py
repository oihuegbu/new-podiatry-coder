...
suppresses_clause (string, optional)
    The finding CLAUSE this suppression was verified against (scrubber
    findings carry a machine `clause` field). Matching is exact: the
    rule retires ONLY the clause it names; a rule without this field
    matches only findings that carry no clause. Never name a clause the
    adjudicated dispute did not verify.
...
also_suppresses_validator_categories (list of strings, optional)
    ...
    so the record never ships the advisory both suppressed and active.
    List a category ONLY when the adjudicated verdict covers its
    assertion too.
